...
/**
    ...
    */
    public boolean finish(){
        for (int i = this.nextInt; i < this.width*this.height; i++){ // For every slot in the array
            if (stillPossible(true)){ //see if can be extended by ture
                setNext(true);
            }
            else if(stillPossible(false)){ // see if can be extended by false
                setNext(false);
            }else{ // cannot be extended
                return(false);
            }
        }
        if (isSuccessful()){ // after fully extended, check valdidity
            return(true);
        }else{ // Not valid
            return(false);
        }
