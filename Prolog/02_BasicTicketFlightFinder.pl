...
% Task Given: With the provided database, create a function which can determine all flights from one location to another
...
findflight(A,B,X) :- flight(A,B,X,_).

arrival(flight(A, B), X) :- findflight(A,B,X).
